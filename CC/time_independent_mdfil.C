...
class DSTFill:public DST{
	public:
		...
		TF1 *frt;		
		DSTFill(TString fn, TString ofn, Int_t mode,Int_t bartel_count):fBadRTI(0),fDMW(1),fMCW(1),fMCpl(1){
			fMode = mode%10;
			IsMC  = mode/10;

			TFile *file = new TFile("~/private/workspace_cenk/he_flux_new/root/MCW.root");
			frt = (TF1*)file->Get("spfun"); 
			file->Close();
...
Double_t DSTFill::GetMCW(Double_t rmc,Double_t par[10]){
	if (!IsMC) return 1;
	
	enum { N = 8 };
	Double_t xn[N]    = {3,5,10,20,50,100,300,1000 };
	Double_t bb[2] 	  = {0, 0};
//	Double_t par[N+2] = {0.687,  0.385, -0.186, -0.910, -1.943, -2.779, -4.298, -6.119, -1.094, -3.509};
	
	TF1 *fun = new TF1("spfun", SplFit::SpFunc, 2.97, 2000, N*2+2);
	...
	if(rmc<=2.97) return fun->Eval(2.97);
	else if(rmc>=2000) return fun->Eval(2000);
	else return fun->Eval(rmc);
}	
Double_t DSTFill::GetMCW_old(Double_t rmc){
	if (!IsMC) return 1;
	...
	*/
		...
		// Copy fitting output from rtfit.C
		Double_t par[N+2] = { 0.121,  0.202, -0.231, -1.049, -2.092, -2.903, -3.694, -4.736,  2.245, -2.546};

		frt = new TF1("spfun", SplFit::SpFunc, 1, 3000, N*2+2);
...
void DSTFill::Init(){
	...
	long unsigned int TimeBegin = 1307750400; //Date2011.06.11;
...
void DSTFill::EventLoop(){
	...
	// Read MCW from list
	ifstream weight_file ("/afs/cern.ch/user/c/ctuysuz/private/workspace_cenk/he_flux_new/root/weight_list.txt");
	string weight;
	Double_t par[10];
	int l = 0;
	cout << "Bartel: " << bartel_count << endl;
	while(l < (bartel_count+1))
	{
		for(int i=0;i<10;i++)	
		{
	...
	for(Int_t i=0;i<Nent;i++)
	{
		...
		//GetEntry("header");	
		//Int_t time_ = fHeader.utime;
		//if((time_ <   1305855336)  || (time_ > (1305855336+79600000)) )  continue;//Use data only before Nov 26th 2013
		
		//Reweighting of MC Event		
		if(IsMC)
		{
			GetEntry("mcinfo");
		...
		//if(fHeader.utime < TimeBegin) continue; // select time  
...
Int_t mdfil(Int_t RID, TString num,Int_t bartel_count)
{
	...
	{
		// Read from list
		ifstream myfile ("/afs/cern.ch/user/c/ctuysuz/public/file_list_mc.txt");
		string line;
		int i = 0;
		while(i!=RID+1)
		{
	...
	else if(mode == 20) // if MC
	{
		// Read from list
		ifstream myfile ("/afs/cern.ch/user/c/ctuysuz/public/file_list_mc2.txt");
		string line;
		int i = 0;
		while(i!=RID+1)
		{
		...
		sdn = "~/../../../../../eos/ams/user/s/selu/mdst/He121/";
		sfn = TString(line);
		ofn +=sfn;
	}
	...
	{
		// Read from list
		ifstream myfile ("/afs/cern.ch/user/c/ctuysuz/public/file_list_iss.txt");
		string line;
		int i = 0;
		while(i!=RID+1)
		{
